...
import javatest01.{KafkaModel, SinkSumModel}
 ...
 */
object MemoryStateBackendTest01 {
...
//    val backend = new FsStateBackend("file:///F:/myflinkstudy/FlinkStudy/data/apitest/MemoryStateBackendTest01")

    env.setStateBackend(backend.asInstanceOf[StateBackend])

        val input = env.addSource(new MemoryStateBackendTestSource01())
//    val input = env.socketTextStream("localhost", 7777)
    val output = input.keyBy(_.substring(12)).process(new MemoryStateBackendTestKeyed001())
    //    input.print("qqq=")
    output.print("www=")
    env.execute()
  }
...
class MemoryStateBackendTestSource001() extends SourceFunction[String]{
...
class MemoryStateBackendTestKeyed001 extends KeyedProcessFunction[String,String,String]{

  lazy val lastValueState: ValueState[SinkSumModel] = getRuntimeContext.getState(new ValueStateDescriptor[SinkSumModel]("last-temp", classOf[SinkSumModel]))


  override def processElement(value: String, ctx: KeyedProcessFunction[String, String, String]#Context, out: Collector[String]): Unit = {

    val model = new SinkSumModel("111234567890"*10000000, "222", new KafkaModel("qqq", "www"))
    lastValueState.update(model)
    out.collect(value)
